...
def process_logs():
    ...
    # Detect alerts
    created_alerts = detect_logs_and_generate_alerts_for_selected_logs(logs_to_process)
    ...
    record_process_run(start_time, end_time, total_logs, total_alerts)  # optional DB persistence

    return jsonify({
        "message": f"Processed {total_logs} logs, created {total_alerts} alerts.",
        "start_time": start_time.isoformat(),
        "end_time": end_time.isoformat(),
        "duration_seconds": (end_time - start_time).total_seconds()
    })

@dashboard_bp.route("/logs/unprocessed", methods=["GET"])
def get_unprocessed_logs():
    """
    Returns JSON of unprocessed logs (processed=False)
    Supports optional limit and offset for pagination.
    """
    limit = int(request.args.get("limit", 50))
    offset = int(request.args.get("offset", 0))

    logs = LogEntry.query.filter_by(processed=False)\
    ...
    logs_list = [
        {
            ...
            "agent_name": log.source,
            "log_type": log.log_type,
            # Fallback to raw_log if message is empty
            "message": log.message if log.message else (json.dumps(log.raw_log) if log.raw_log else "N/A")
        } for log in logs
    ]

    total = LogEntry.query.filter_by(processed=False).count()
    return jsonify({"unprocessed_logs": logs_list, "total": total})
...
def process_unmatched_logs():
    ...
    return jsonify({
        "message": f"Processed {total_logs} unmatched logs, created {total_alerts} alerts.",
        "start_time": start_time.isoformat(),
        "end_time": end_time.isoformat(),
        "duration_seconds": (end_time - start_time).total_seconds()
    })

def record_process_run(run_type, start_time, end_time, duration, total_logs, total_alerts):
    """
    Example function to record process metrics.
    You can adapt this to save in DB, a file, or audit table.
    """
    print(f"[{run_type}] Start: {start_time}, End: {end_time}, Duration(s): {duration}, "
          f"Logs: {total_logs}, Alerts: {total_alerts}")
...
def get_dashboard_context():
    ...
    # --- Filter agents with at least one alert ---
    agents_with_alerts = [
        agent for agent, count in alert_counts.items() if count > 0
    ]
    ...
    for agent in agents_with_alerts:
    ...
    total_agents_with_alerts = len(agents_with_alerts)
    ...
    context = {
        "username": username,
        "role": role,
        "total_agents": total_agents_with_alerts,
        "logs": get_log_summary(),
        "alerts": get_alert_summary(),
        "agent_names": agents_with_alerts,
        "alert_counts": {a: alert_counts[a] for a in agents_with_alerts},
